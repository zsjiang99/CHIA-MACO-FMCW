...
export type Implementation={state:string;design_id:string;current:string|null;detail?:{stage:string;completed:number;total:number};stages:Record<'verify'|'synth'|'layout',{state:string;error?:string;result?:Record<string,unknown>}>};
export type RadarData={run:string;entries:Entry[];progress:Progress;implementation?:Implementation|null;workload?:import('./studio').Workload;baseline_comparable?:boolean;baseline:{id:string;design:Plan;frame_estimate:Entry['frame_estimate']};best_validated_feasible:null};
export const cycles=(e:Entry)=>e.frame_estimate.estimated_cycles;
export const bestIndex=(entries:Entry[],objective:'cycles'|'spm_energy'='cycles')=>entries.reduce((best,e,i)=>{
 const score=(v:Entry)=>objective==='spm_energy'?(v.memory?.dynamic_energy_uj??Infinity):(cycles(v)??Infinity);
