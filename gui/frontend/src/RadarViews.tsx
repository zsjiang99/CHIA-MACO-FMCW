...
export function ArchitectureModeling({entry,selected}:{entry:Entry;selected:number}){
 ...
 return <section className="flow-panel architecture-modeling"><header><h2>CGRA Modeling</h2><span>Tile {selected}</span></header>
  <div className="parameter-top"><label>Rows <output>{n}</output></label><label>Columns <output>{n}</output></label><label>Per-bank SRAM <output>{entry.architecture?`${entry.architecture.memory.bank_kib} KiB`:'—'}</output></label><label>Config memory <output>{configMemory??'—'}</output></label></div>
