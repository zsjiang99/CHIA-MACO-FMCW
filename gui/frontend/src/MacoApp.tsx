...
function MacoPanel({entry,workload,prompt,setPrompt,method,setMethod,objective,setObjective,onRun,busy,error,progress,implementation,onExport,archived}:{entry:Entry;workload?:Workload;prompt:string;setPrompt:(s:string)=>void;method:SearchMethod;setMethod:(value:SearchMethod)=>void;objective:'cycles'|'energy';setObjective:(value:'cycles'|'energy')=>void;onRun:()=>void;busy:boolean;error:string;progress?:Progress;implementation?:Implementation|null;onExport:()=>void;archived:boolean}){
 ...
 return <section className={`flow-panel maco-panel ${archived?'archived':''}`}><header><h2>MACO Agent Decisions</h2><i className={busy?'busy':''}/></header>
  {archived?<section className="design-intent archive-intent"><h3>Paper experiment · seed 37</h3><p>One CGRA shared across five FMCW kernels. The agents propose array size and compiler unroll; CHIA returns actual mapper feedback.</p><a href="/?mode=live">Configure a new search ↗</a></section>:<form className="design-intent" onSubmit={e=>{e.preventDefault();onRun();}}><h3>Design Intent</h3><label htmlFor="workload-description">Describe your workload and design requirements</label><textarea id="workload-description" value={prompt} onChange={e=>setPrompt(e.target.value)}/><label>Exploration method</label><div className="method-select" role="group" aria-label="Exploration method"><button type="button" title="Four-agent hardware/compiler co-design" className={method==='full_maco'?'active':''} onClick={()=>setMethod('full_maco')}>Full MACO</button><button type="button" title="Architecture search with fixed compiler settings" className={method==='hardware_only'?'active':''} onClick={()=>setMethod('hardware_only')}>Hardware only</button><button type="button" title="One-agent hardware/compiler co-design" className={method==='single_agent'?'active':''} onClick={()=>setMethod('single_agent')}>Single agent</button></div><div className="objective-select" role="group" aria-label="Optimization objective"><button type="button" className={objective==='cycles'?'active':''} onClick={()=>setObjective('cycles')}>Performance</button><button type="button" className={objective==='energy'?'active':''} onClick={()=>setObjective('energy')}>Energy</button></div><button className="run-button" disabled={busy||!prompt.trim()}>{busy?'Exploration running…':'Run exploration'}</button>{error&&<p className="inline-error">{error}</p>}</form>}
