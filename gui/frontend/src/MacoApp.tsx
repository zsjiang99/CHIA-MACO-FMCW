...
function MacoPanel({entry,workload,prompt,setPrompt,objective,setObjective,onRun,busy,error,progress,implementation,onExport,archived}:{entry:Entry;workload?:Workload;prompt:string;setPrompt:(s:string)=>void;objective:'cycles'|'energy';setObjective:(value:'cycles'|'energy')=>void;onRun:()=>void;busy:boolean;error:string;progress?:Progress;implementation?:Implementation|null;onExport:()=>void;archived:boolean}){
 ...
 const flowState=(name:string)=>name==='explore'?(implementation?'passed':busy?'running':events.some(e=>e.kind==='run_finished')?'passed':'pending'):(implementation?.stages[name as 'verify'|'synth']?.state??'pending');
 ...
 const current=mainDone?'Flow complete':implementation?.current?steps.find(([key])=>key===implementation.current)?.[1]:busy?'Explore':implementation?.state==='failed'?'Flow stopped':'Ready to explore';
 return <section className={`flow-panel maco-panel ${archived?'archived':''}`}><header><h2>MACO Agent Decisions</h2><i className={busy?'busy':''}/></header>
  {archived?<section className="design-intent archive-intent"><h3>Paper experiment · seed 37</h3><p>One CGRA shared across five FMCW kernels. The agents propose array size and compiler unroll; CHIA returns actual mapper feedback.</p><a href="/?mode=live">Configure a new search ↗</a></section>:<form className="design-intent" onSubmit={e=>{e.preventDefault();onRun();}}><h3>Design Intent</h3><label htmlFor="workload-description">Describe your workload and design requirements</label><textarea id="workload-description" value={prompt} onChange={e=>setPrompt(e.target.value)}/><div className="objective-select" role="group" aria-label="Optimization objective"><button type="button" className={objective==='cycles'?'active':''} onClick={()=>setObjective('cycles')}>Performance</button><button type="button" className={objective==='energy'?'active':''} onClick={()=>setObjective('energy')}>Energy</button></div><button className="run-button" disabled={busy||!prompt.trim()}>{busy?'Exploration running…':'Run MACO exploration'}</button>{error&&<p className="inline-error">{error}</p>}</form>}
  {archived?<section className="design-flow round-decisions"><h3>Feedback changed the choice</h3>{progress?.rounds?.map(round=><div className="round-decision" key={round.iteration}><b>R{round.iteration}</b><span>Judge: {round.llm_choice.tile_size}<br/>Tool best: {round.measured_winner?.design.tile_size??'—'}</span><strong>{round.measured_winner?.frame_estimate.estimated_cycles==null?'—':`${fmt(round.measured_winner.frame_estimate.estimated_cycles/1e6,2)}M`}</strong></div>)}</section>:<section className="design-flow"><h3>Design Flow</h3><div className="flow-status" aria-label="Automated design flow">{steps.map(([key,label],i)=><div key={key} className={`flow-step ${flowState(key)}`}><b>{flowState(key)==='passed'?'✓':flowState(key)==='failed'?'×':i+1}</b><span>{label}</span></div>)}</div><div className="flow-progress"><progress aria-label="End-to-end flow progress" value={percentage} max="100"/><span>{current}</span></div>{implementation?.detail&&<p className="flow-detail">{implementation.detail.stage}</p>}{!mainDone&&implementation?.state==='failed'&&<p className="flow-error">{implementation.stages[implementation.current as 'verify'|'synth']?.error}</p>}</section>}
  <section className="selected-design"><h3>Best Mapper-Evaluated Plan</h3><div className="primary-result"><span>{primaryLabel}</span><strong>{primaryValue}<small>{primaryUnit}</small></strong></div><dl><div><dt>Array</dt><dd>{entry.design.tile_size}</dd></div><div><dt>Frame estimate</dt><dd>{cycles(entry)==null?'—':`${fmt(cycles(entry)!/1e6,2)} M cycles`}</dd></div>{archived?<><div><dt>Kernel unroll</dt><dd>[{entry.design.unroll_factors?.join(', ')??'—'}]</dd></div><div><dt>Hardware status</dt><dd>Not validated</dd></div></>:<><div><dt>FU placement</dt><dd>{entry.architecture?.fu_profile==='custom'?'Per tile':entry.architecture?.fu_profile??'generic'}</dd></div><div><dt>Memory</dt><dd>{entry.architecture?`${entry.architecture.memory.banks} × ${entry.architecture.memory.bank_kib} KiB`:'—'}</dd></div></>}</dl>{!archived&&<button className="secondary-button" onClick={onExport}>Export architecture</button>}</section>
...
function ImplementationPanel({entry,implementation,hardware,run}:{entry:Entry;implementation?:Implementation|null;hardware?:HardwareJob|null;run:string}){
 ...
 const task=(name:'verify'|'synth')=>auto?<div className="hardware-progress"><span>{auto.stages[name].state==='running'?(auto.current===name?auto.detail?.stage??'Running…':'Running…'):auto.stages[name].state==='passed'?'Completed':auto.stages[name].state==='failed'?'Failed':'Queued'}</span>{auto.stages[name].error&&<span className="hardware-error">{auto.stages[name].error}</span>}{auto.stages[name].state!=='pending'&&<a className="job-log" href={`/api/jobs/${run}/implementation/${name}/log`} target="_blank">Log ↗</a>}</div>:active?.kind===`cgra-${name}`?<><div className="hardware-progress">{active.state==='running'&&<progress value={active.progress?.completed??0} max={active.progress?.total??4}/>}<span>{active.state==='running'?(active.progress?.stage??'Starting CGRA-Flow'):(active.result?.status==='passed'?'Completed':'Task failed')}</span></div><a className="job-log" href={`/api/jobs/${active.id}/log`} target="_blank">Task log ↗</a>{error&&<p className="hardware-error">{error}</p>}</>:name==='synth'&&hardware?.kind==='cgra-synth'?<div className="hardware-progress"><span>Independent job · {hardware.state==='running'?(hardware.progress?.stage??'starting'):hardware.state==='completed'?'completed':'failed'}{hardware.state==='running'&&hardware.started_at?` · ${Math.floor((Date.now()/1000-hardware.started_at)/60)} min`:''}</span><a className="job-log" href={`/api/jobs/${hardware.id}`} target="_blank">Job status ↗</a></div>:null;
 ...
 return <section className="implementation-stack"><section className="flow-panel verification-panel"><header><h2>RTL Generation &amp; Verification</h2></header>
  ...
  <section className="flow-panel report-panel"><header><h2>Area &amp; Energy</h2></header><div className="report-values"><Field label="CGRA logic area" value={coreArea==null?'—':`${fmt(coreArea,4)} mm²`}/><Field label="SRAM area" value={sramArea==null?'—':`${fmt(sramArea,4)} mm²`}/><Field label="Dynamic energy/frame" value={entry.energy?.total_dynamic_energy_uj==null?'—':`${fmt(entry.energy.total_dynamic_energy_uj,2)} µJ`}/></div>{task('synth')}{!auto&&<button className="panel-action" disabled={busy} onClick={()=>void launch('synth')}>{running('synth')?'Synthesizing…':'Synthesize'}</button>}</section>
  <section className="flow-panel layout-panel"><header><h2>Layout</h2><span>Optional</span></header>{layout?.layout&&(auto||layout.job_id)?<img className="cgra-layout" src={`/api/jobs/${layoutStage?.job_id??layout.job_id??run}/layout`} alt="OpenROAD CGRA layout"/>:<><div className="layout-fields"><Field label="Platform" value="ASAP7"/><Field label="Flow configuration" value="config.mk"/><Field label="Timing constraints" value="constraint.sdc"/></div><p className="layout-note">{mainDone?'Run layout when needed.':'Available after RTL verification and synthesis.'}</p></>}<button className="panel-action" disabled={!mainDone||busy||layoutStage?.state==='passed'||layoutStage?.state==='running'} onClick={()=>void launch('layout')}>{running('layout')||layoutStage?.state==='running'?'OpenROAD running…':layoutStage?.state==='passed'?'Layout complete':'Run layout'}</button>{layoutStage?.state!=='pending'&&layoutStage&&<div className="hardware-progress"><span>{layoutStage.state==='running'?'OpenROAD running…':layoutStage.state==='passed'?'Completed':'Failed'}</span>{layoutStage.error&&<span className="hardware-error">{layoutStage.error}</span>}{layoutStage.job_id&&<a className="job-log" href={`/api/jobs/${layoutStage.job_id}/log`} target="_blank">Task log ↗</a>}</div>}{!layoutStage?.job_id&&layoutJob&&<div className="hardware-progress"><span>{layoutJob.state==='running'?'OpenROAD running…':layoutJob.state==='completed'?'Completed':'Failed'}</span><a className="job-log" href={`/api/jobs/${layoutJob.id}/log`} target="_blank">Task log ↗</a></div>}{error&&<p className="hardware-error">{error}</p>}</section></section>;
}
...
function App(){
 ...
 const[prompt,setPromptState]=useState(defaultPrompt),[objective,setObjective]=useState<'cycles'|'energy'>('cycles'),[inputError,setInputError]=useState(''),[interpreting,setInterpreting]=useState(false);
