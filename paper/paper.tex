...
The reusable CHIA node accepts a validated kernel/array/unroll candidate and returns a structured mapping record. A separate frame-cost reducer combines five such records. This boundary lets enumeration and the MACO policy share exactly the same compiler, mapper, failure handling, and aggregation without passing reference-search results to the agents.
