...
class CrossbarRTL(Component):
  ...
  def construct(s,
    ...
    s.prologue_increment = Wire(num_inports)
    ...
    def update_prologue_counter():
      if s.reset | s.clear:
        ...
        for i in range(num_inports):
          if s.prologue_increment[i] & \
             (s.prologue_counter[s.ctrl_addr_inport][i] < \
              s.prologue_count_wire[s.ctrl_addr_inport][i]):
            s.prologue_counter[s.ctrl_addr_inport][i] <<= \
              s.prologue_counter[s.ctrl_addr_inport][i] + 1

    @update
    def update_prologue_increment():
      s.prologue_increment @= 0
      for i in range(num_inports):
        for j in range(num_outports):
          if s.recv_opt.rdy & \
             (s.in_dir[j] > 0) & \
             (s.in_dir_local[j] == i):
            s.prologue_increment[i] @= 1
