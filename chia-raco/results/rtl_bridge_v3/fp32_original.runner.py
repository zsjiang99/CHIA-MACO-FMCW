...
from VectorCGRA.lib.opt_type import OPT_FADD, OPT_FSUB, OPT_FMUL, OPT_LT
...
def main():
    ...
    guard = np.array([0, 1, 2, 3, 4, 0x7fffffff, 0x80000000, 0xfffffffc, 0xffffffff], dtype=np.uint32)
    cases.append(("cfar_unsigned_guard", CompRTL, OPT_LT, guard,
                  np.full_like(guard, 3), (guard < 3).astype(np.uint32)))
    ...
    for name, unit, opcode, x, y, expected in cases:
        ...
        try:
            ...
            result = dict(name=name, status="passed", samples=len(x), comparison="exact uint32" if name == "cfar_unsigned_guard" else "bit-exact float32 including sign of zero")
